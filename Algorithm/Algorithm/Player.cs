...
namespace Algorithm
{
    ...
    class Player
    {
        ...
        void CalcPathFromParent(Pos[,] parent)
        {
            ...
            _points.Add(new Pos(y, x)); // 시작점 리스트에 추가해주기 
            _points.Reverse();
        }

        //우수법 >> 오른속으로 미로벽을 계속 짚고 가다보면 언젠간 길이 나온다.
        void RightHand()
        {
            ...
            {
                //우수법 >> 미로를 오른손으로 짚으면서 가면 언젠가는 출구가 나온다?

                //현재 바라보는 방향을 기준으로 오른쪽으로 갈수 있는지 확인 
                if (_board.Tile[PosY + rightY[_dir], PosX + rightX[_dir]] == Board.TileType.Empty)
                {
                    //오른쪽으로 90도 회전 
                    _dir = (_dir - 1 + 4) % 4; // 개어렵다 이거 
                ...
                //현재 바라보는 방향을 기준으로 전진 할 수 있는지 확인
